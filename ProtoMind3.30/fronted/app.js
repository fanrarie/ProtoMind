...
// 图片导入按钮事件
document.getElementById('import-image-btn').addEventListener('click', importImage);
    ...
    for (let i = 0; i < edgePixels.length; i++) {
        ...
        if (radius > 0) {
            ...
            for (const circle of circles) {
                const dist = Math.sqrt(Math.pow(circle.x - center.x, 2) + Math.pow(circle.y - center.y, 2));
