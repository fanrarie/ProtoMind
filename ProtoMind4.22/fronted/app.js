...
document
  ...
  .addEventListener("click", async () => {
    const trainPrompt = "预训练模型";
    promptInput.value = trainPrompt; // 在输入框内追加内容
    const formData = new FormData();
...
`;
    promptInput.value = desPrompt; // 在输入框内追加内容
  });
...
document
  ...
  .addEventListener("click", function () {
    ...
    promptInput.value = clearedMessage; // 清空并追加清除确认信息
  });
// 生成 IR 逻辑
...
document
  ...
  .addEventListener("click", async () => {
    try {
      ...
      if (result.data.fsmJson) {
        initializeStateMachine(result.data.fsmJson);
      } else {
        throw new Error("后端响应缺少 fsmJson 数据");
      }
...
document
  ...
  .addEventListener("click", async () => {
    try {
      ...
      const formContainer = document.querySelector(".horizontal-params-form");
      if (!formContainer) {
        throw new Error("未找到通信参数设置表单");
      }

      // 只收集 generatedFieldKeys 中的字段
      const selections = {};
      ...
      if (!response.ok) {
        const error = await response.text();
        throw new Error(`生成数据包失败 (${response.status}): ${error}`);
      }
      ...
      if (result.packets) {
        ...
        addStatusOutput("生成成功，但无数据包返回", "success");
      }
...
function displayPacketDetails(packet) {
  clearTimeout(detailsTimeout); // 清除上一个定时器
