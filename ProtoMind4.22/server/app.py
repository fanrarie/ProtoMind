...
def gen_fsm_wt_info(pit_path):
    """生成FSM信息及图片"""
    data_dict = fsm_explan.GEN_FSM(pit_path)
    return data_dict
...
def ret_pcap_info(pcap_file):
    ...
    for idx, packet in enumerate(packets, 1):
        packet_info = {
            'no': idx,
            'time': datetime.fromtimestamp(float(packet.time)).strftime('%H:%M:%S'),  # 使用 datetime 而非 datetime.datetime
            'length': len(packet),
            'details': {}
        }
...
def controller():
    ...
    # 2. PIT命令        
        ...
        if not action:
            return Response("操作缺失", status=400)
        if action ==1:
            pass
        elif action ==2:
            try:
    ...
    # 4. gen_pack命令
        ...
        try:
            ...
            packet=jsonify({"packets": ret_pcap_info(pcap_path)})
            return packet
        except Exception as e:
            print(f"后端错误: {str(e)}")
    ...
    # 5. PROCESS_XML命令
        ...
        try:
            ...
            return jsonify({"error": f"处理 XML 失败: {str(e)}"}), 500
