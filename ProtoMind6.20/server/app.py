...
def ret_pcap_info(pcap_file):
    ...
    try:
        ...
        for idx, packet in enumerate(scapy_packets, 1):
            packet_info = {
                'no': idx,
                'time': float(packet.time),  # 确保时间是可序列化的
                'length': len(packet),
...
def controller():
    ...
    # 1. RFC命令
    if command == "RFC":
        global SELECTED_RFC,XML_TYPE
        xml_type = {
            'mqtt-v3.1.1.txt': 'mqttIR.xml',
            'rfc1034.txt': 'dnsIR.xml',
            'modbus.txt':'modbusIR.xml'
        }
        rfc_name = request.form.get("rfcName")
        if not rfc_name:
            return Response("RFC名称缺失", status=400)
        if rfc_name not in xml_type:
            XML_TYPE = f"fsm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xml"
        else:
            XML_TYPE = xml_type[rfc_name]
    ...
    # 3. GEN_PACK命令 - 修改后的部分
        ...
        try:
            ...
            # 确保端口是数字
            if XML_TYPE != "modbusIR.xml":
                try:
                    selections['target_port'] = int(selections['target_port'])
                except ValueError:
                    return jsonify({'error': '端口必须是数字'}), 400
            else:
                selections['target_port'] = ''
            
            # 根据 xml_file 确定协议类型
            protocol_type = XML_TYPE.replace('.xml','')##########
            
            # 生成 PCAP
            logger.debug(f"生成 PCAP，xml_file: {XML_TYPE}, selections: {selections}")
            pcap_path = GEN_PACK(
                xml_file=XML_TYPE,
            ...
            # 解析 PCAP
            logger.debug(f"解析 PCAP 文件: {pcap_path}")
            packets = ret_pcap_info(pcap_path)
            logger.debug(f"解析到的 packets: {packets}")
            ...
            logger.error(f'生成 PCAP 错误: {str(e)}')
            import traceback
