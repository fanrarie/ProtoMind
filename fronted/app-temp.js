...
function updateStatusMessage(page) {
  ...
  switch (page) {
    case "upload":
      ...
      break;
    case "model-prompt":
      message = "请选择大模型并确认提示词，然后点击“生成 IR”。";
      break;
    case "xml-editor":
      message = "请编辑 XML 内容并点击“确定”提交。";
      break;
    case "result":
      message = "请查看图像并完成选择，然后点击“确认”。";
      break;
    case "packet-viewer":
...
document.addEventListener("mousemove", (e) => {
  ...
  mainContent.style.height = `calc(100vh - 60px - ${newHeight}px)`;
});
...
generateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    document.getElementById("status-message").textContent =
      "PROTOMIND>> 错误：生成 IR 文档时出错，请稍后重试。";
  }
});
...
document
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      document.getElementById("status-message").textContent =
        "PROTOMIND>> 错误：发送 XML 数据时出错，请稍后重试。";
    }
  });
  ...
  .addEventListener("click", async () => {
    ...
    try {
      ...
      document.getElementById(
        "status-message"
      ).textContent = `PROTOMIND>> 错误：提交失败 - ${error.message}`;
    }
  });
